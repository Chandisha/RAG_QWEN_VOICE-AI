...
import time
import uuid
from threading import Thread # Added for streaming implementation
...
from transformers.generation.streamers import TextIteratorStreamer # Added for streaming
...
# --- 3. RAG Query Function (Streaming Text Generation) ---
def rag_query(query, index, embedder, llm_model, llm_tokenizer, raw_chunks, top_k=5):
    """
    Performs the RAG query: Embed query, search index, retrieve context, and starts
    a streaming generation process. Returns a generator/streamer object and context chunks.
    """
    print(f"Embedding query: '{query}'...")
    # Embed the query (CPU operation)
    ...
    # --- Setup Streaming ---
    # Create the streamer and the thread
    streamer = TextIteratorStreamer(llm_tokenizer, skip_prompt=True, skip_special_tokens=True)

    # Arguments for the generate call
    # Note: Pass input_ids as the first positional argument for model.generate
    ...
    # Start the generation in a separate thread so we can iterate over the streamer in the main thread
    thread = Thread(target=llm_model.generate, kwargs=generation_kwargs)
    thread.start()

    # The streamer is an iterable that yields the generated tokens
    return streamer, context_chunks
...
if __name__ == "__main__":
    ...
    print(f"\n--- 6. Running RAG Query with STT Result (STREAMING) ---")
    print(f"Query (from STT): {stt_query}")
    
    # rag_query now returns a streamer (generator) and context
    streamer, context_list = rag_query(stt_query, index, embedder, llm_model, llm_tokenizer, raw_chunks)
    
    print("\n✅ Streaming Generated Answer:")
    
    final_answer_parts = []
    # Loop over the streamer to get tokens in real-time
    for new_text in streamer:
        print(new_text, end="", flush=True) # Print the token immediately
        final_answer_parts.append(new_text)
        
    # Reconstruct the final answer from all streamed parts
    final_answer = "".join(final_answer_parts).strip()
    
    print("\n\n--- Stream finished. ---\n")
    
    # STAGE 3: TTS (Answer to Audio) - Note: This must wait for the full answer
    if final_answer:
        text_to_audio(final_answer, TTS_OUTPUT_FILE)
    else:
        print("Skipping TTS: No response generated by the LLM.")
